...
namespace ni {
    ...
    template<auto type, bool IsCached = true, class ... Args>
    constexpr auto make_i(Args && ... args) {
        ...
        if constexpr (std::is_same_v<decltype(type), Type1D>) {
            return ni::i_1d<type, container_type, IsCached>(std::forward<Args>(args)...);
        } else if constexpr (std::is_same_v<decltype(type), Type2DScat>) {
            return ni::i_2d_scat<type, container_type, IsCached>(std::forward<Args>(args)...);
        } else if constexpr (std::is_same_v<decltype(type), Type2DRBF>) {
            return ni::i_2d_rbf<type, container_type, IsCached>(std::forward<Args>(args)...);
        } else {
