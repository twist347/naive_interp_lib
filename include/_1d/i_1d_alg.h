...
namespace interp::detail {
    ...
    auto apply_transform(XIter x_first, XIter x_last, DestIter dest_first, UnaryOp unary_op) -> void {
        const auto x_sz = std::distance(x_first, x_last);
        utils::custom_transform(x_sz, THRESHOLD_VALUE, x_first, x_last, dest_first, unary_op);
    }
