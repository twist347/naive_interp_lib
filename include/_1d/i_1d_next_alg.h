...
namespace interp {
    ...
    auto next(
        ...
        if (!p.bounds_check && !p.extrapolate) {
            detail::next_pure_impl(x_first, x_last, xp_first, xp_last, yp_first, dest_first, p);
        } else if (p.bounds_check && !p.extrapolate) {
            detail::next_bounds_check_impl(x_first, x_last, xp_first, xp_last, yp_first, dest_first, p);
        } else if (!p.bounds_check && p.extrapolate) {
            detail::next_extr_impl(x_first, x_last, xp_first, xp_last, yp_first, dest_first, p);
        } else {
