...
namespace interp {
    ...
    namespace detail {
        ...
        constexpr auto direct_prev_calc_(XpIter xp_first, XpIter xp_last, YpIter yp_first, Value xi) -> Value {
    ...
    auto prev(
        ...
        if (!p.bounds_check && !p.extrapolate) {
            detail::prev_pure_impl(x_first, x_last, xp_first, xp_last, yp_first, dest_first);
        } else if (p.bounds_check && !p.extrapolate) {
            detail::prev_bounds_check_impl(x_first, x_last, xp_first, xp_last, yp_first, dest_first, p);
        } else if (!p.bounds_check && p.extrapolate) {
            detail::prev_extr_impl(x_first, x_last, xp_first, xp_last, yp_first, dest_first);
        } else {
