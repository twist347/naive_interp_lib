...
TEST(PrevInterp, Base) {
    ...
    auto interp = ni::make_i<ni::Type1D::Prev>(xp, yp);
...
TEST(NextInterp, Base) {
    ...
    auto interp = ni::make_i<ni::Type1D::Next>(xp, yp);
    const auto y = interp(x);
    ASSERT_TRUE(arrays_eq(y, expected));
}
...
TEST(NearestNeighbourInterp, Base) {
    ...
    auto interp = ni::make_i<ni::Type1D::NearestNeighbour>(xp, yp);
    const auto y = interp(x);
    ASSERT_TRUE(arrays_eq(y, expected));
}
...
TEST(LinearInterp, Base) {
    ...
    auto interp = ni::make_i<ni::Type1D::Linear>(xp, yp);
    const auto y = interp(x);
    ASSERT_TRUE(arrays_eq(y, expected));
}
...
TEST(QuadraticInterp, Base) {
    ...
    auto interp = ni::make_i<ni::Type1D::Quadratic>(xp, yp);
    const auto y = interp(x);
    ASSERT_TRUE(arrays_eq(y, expected));
}
...
TEST(CubicInterp, Base) {
    const std::vector<double> xp{1, 2, 3, 4, 5}, yp{1, 4, 9, 16, 25}, x{2, 2.5, 4.5}, expected{4, 6.25, 20.25};
    auto interp = ni::make_i<ni::Type1D::Cubic>(xp, yp);
    const auto y = interp(x);
    ASSERT_TRUE(arrays_eq(y, expected));
}
...
TEST(CubicSplineInterp, Base) {
    ...
    auto interp = ni::make_i<ni::Type1D::CubicSpline>(xp, yp);
    const auto y = interp(x);
...
TEST(AkimaInterp, Base) {
    const std::vector<double> xp{1, 2, 3, 4, 5}, yp{1, 4, 9, 16, 25}, x{2, 2.5, 4.5}, expected{4, 6.25, 20.25};
    auto interp = ni::make_i<ni::Type1D::Akima>(xp, yp);
    const auto y = interp(x);
    ASSERT_TRUE(arrays_eq(y, expected));
}
